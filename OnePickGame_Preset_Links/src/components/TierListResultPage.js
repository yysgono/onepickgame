...
if (isAdmin) {
  ...
  const openPresetTierLists = (excludeCurrent = false) => {
    ...
    if (excludeCurrent) params.set("exclude", String(tierList.id || id));
...
>
  ...
  <div style={{display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "baseline", gap: 12, marginTop: 24, color: "#8fa6c3", fontSize: isMobile ? 12 : 14, fontWeight: 700}}>
    <div>
    ...
    {canBrowsePreset && (
      <div style={{marginLeft: "auto", minWidth: 0, maxWidth: "100%", textAlign: "right"}}>
        ...
        <button type="button" onClick={() => openPresetTierLists(false)} style={{border: 0, background: "transparent", color: "#64d8ff", padding: 0, font: "inherit", textDecoration: "underline", cursor: "pointer", overflowWrap: "anywhere", textAlign: "inherit"}}>
          {sourcePresetName || t("tierList.result.sourcePresetFallback")}
        </button>
      </div>
    )}
  </div>
...
objectPosition: "center",
                ...
                {canBrowsePreset && (
                  <button type="button" onClick={() => openPresetTierLists(true)} style={{marginTop: 10, width: "100%", padding: "13px 14px", borderRadius: 13, border: "1px solid #19bfff", background: "#073653", color: "#fff", fontSize: isMobile ? 14 : 15, fontWeight: 900, lineHeight: 1.4, cursor: "pointer"}}>
