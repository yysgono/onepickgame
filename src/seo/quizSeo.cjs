...
const DEFAULT_QUIZ_LANGUAGE = 'ko'; // Stable language for the legacy /quiz URL.
const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const OG_LOCALES = { ko: 'ko_KR', en: 'en_US', ja: 'ja_JP', zh: 'zh_CN', es: 'es_ES', fr: 'fr_FR', vi: 'vi_VN', de: 'de_DE', ru: 'ru_RU', id: 'id_ID', pt: 'pt_BR', hi: 'hi_IN', tr: 'tr_TR', th: 'th_TH', ar: 'ar_AR', bn: 'bn_IN' };
function normalizeLanguage(value) { return String(value || '').toLowerCase().split(/[-_]/)[0]; }
...
function quizLanguages(quiz) {
  ...
  return [...new Set(configured.map(normalizeLanguage))].filter(l => LANGS.includes(l));
}
function socialImage(value, origin = SITE_URL) {
  ...
  try {
    ...
    if (['youtube.com', 'youtube-nocookie.com', 'youtu.be'].includes(host)) {
      ...
      return /^[a-zA-Z0-9_-]{11}$/.test(id || '') ? `https://i.ytimg.com/vi/${id}/hqdefault.jpg` : fallback;
    }
    if (/\.(?:mp4|webm|mov|m4v|m3u8|mp3|ogg)$/i.test(url.pathname)) return fallback;
    return url.href;
  } catch { return fallback; }
}
function getQuizSeo(language, quiz, origin = SITE_URL) {
  const requested = normalizeLanguage(language);
  const lang = LANGS.includes(requested) ? requested : 'en';
  const copy = COPY[lang];
  const name = quiz ? localized(quiz.title_translations, quiz.title, lang) || copy.name : copy.name;
...
module.exports = { COPY, LANGS, SITE_URL, DEFAULT_QUIZ_LANGUAGE, UUID, normalizeLanguage, localized, quizLanguages, socialImage, getQuizSeo };
