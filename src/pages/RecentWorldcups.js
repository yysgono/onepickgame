...
function getRecentIds() {
  // ["id1", "id2", ...]
  return JSON.parse(localStorage.getItem("recent_worldcup_ids") || "[]");
}

export default function RecentWorldcups() {
  ...
  useEffect(() => {
    ...
    // 최근 본 순서 유지, 중복 방지
    const recentWcs = recents
      .map(id => all.find(wc => wc.id === id))
      .filter(Boolean);
    setRecentList(recentWcs);
  }, []);
