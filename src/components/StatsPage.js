...
function ReportButton({ cupId, size = "md" }) {
  ...
  const handleReport = useCallback(async () => {
    ...
    const { error } = await supabase.from("reports").insert([
      { type: "worldcup", target_id: cupId, reporter_id: data.user.id, reason },
    ]);
...
export default function StatsPage({
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      if (customMode && customFrom && customTo) {
        ...
        arr = await fetchWinnerStatsFromDB(selectedCup.id, range);
      } else {
        const since = getSinceDate(period);
        arr = await fetchWinnerStatsFromDB(selectedCup.id, since);
      }
      setStats(arr);
      setLoading(false);
    }
  ...
  // 2) 카드용: 항상 ALL(기간 null + 회원/비회원 합산)로만 1번 로드
  useEffect(() => {
    async function fetchAll() {
      ...
      const allArr = await fetchWinnerStatsFromDB(selectedCup.id, null); // since=null
      setStatsAll(allArr);
    }
    fetchAll();
  }, [selectedCup]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, itemsPerPage, stats, userOnly, period, customMode, customFrom, customTo]);

  // 상위 3명 카드: "항상 ALL" 기준(statsAll 고정)
  const top3 = useMemo(() => {
    return [...statsAll]
      .map((row, i) => ({ ...row, _originIdx: i }))
      .sort((a, b) => {
  ...
  const normalCell = { background: "#fff", color: "#333", padding: "7px 0" };
  ...
  return (
    ...
    >
      ...
      {/* 회원/전체 탭 */}
      <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 10, marginTop: 4 }}>
      ...
      {/* 기간 필터 */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 0, justifyContent: "center", marginBottom: 10 }}>
        {PERIODS.map((p) => (
          <button
        ...
        <button
          style={{ ...periodBtnStyle(customMode), marginRight: 0, background: customMode ? "#e7f7f6" : "#fff" }}
          onClick={() => {
            setCustomMode(true);
            setPeriod(undefined);
          }}
      ...
      <div style={{ width: "100%", overflowX: "auto", marginBottom: 12 }}>
        ...
        >
          ...
          <tbody>
            {loading ? (
              Array.from({ length: 5 }).map((_, i) => <SkeletonTableRow key={i} colCount={7} />)
            ) : pagedStats.length === 0 ? (
              <tr>
      ...
      {showCommentBox && (
        <div style={{ width: "100%", display: "flex", justifyContent: "center", margin: "18px 0 10px" }}>
          <div style={{ width: isMobile ? 320 : 728, height: isMobile ? 100 : 90 }}>
            <AdSlot id="ad-stats-above-comments" provider={provider} width={isMobile ? 320 : 728} height={isMobile ? 100 : 90} />
          </div>
        </div>
      )}
