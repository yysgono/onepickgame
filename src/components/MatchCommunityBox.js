...
export default function MatchCommunityBox({ cupId }) {
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        ...
        "id, user_id, nickname, content, language, status, created_at"
  ...
  async function handleDeleteSuggestion(item) {
    if (!item?.id || !user?.id || item.user_id !== user.id) {
      return;
    }

    const confirmed = window.confirm(
      t("matchCommunity.deleteConfirm", {
        defaultValue: "Delete this suggestion?",
      })
    );
    ...
    if (error) {
      console.error(
        "MatchCommunityBox suggestion delete error:",
        error
      );

      setSuggestionsError(
        t("matchCommunity.deleteError", {
          defaultValue: "Could not delete the suggestion.",
        })
      );
      return;
    }
    ...
    setSuggestions((current) =>
      current.filter(
        (suggestionItem) => suggestionItem.id !== item.id
      )
    );

    setSuggestionCount((current) =>
