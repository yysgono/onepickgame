...
const OTHER_LANGUAGE_TITLE_COPY = {
  ko: { add: "＋ 다른 언어 제목 추가", title: "번역 제목", description: "번역 설명 (선택)", remove: "삭제" },
  en: { add: "＋ Add title in another language", title: "Translated title", description: "Translated description (optional)", remove: "Remove" },
  ja: { add: "＋ 他の言語のタイトルを追加", title: "翻訳タイトル", description: "翻訳説明（任意）", remove: "削除" },
  zh: { add: "＋ 添加其他语言标题", title: "翻译标题", description: "翻译说明（可选）", remove: "删除" },
  es: { add: "＋ Añadir título en otro idioma", title: "Título traducido", description: "Descripción traducida (opcional)", remove: "Eliminar" },
  fr: { add: "＋ Ajouter un titre dans une autre langue", title: "Titre traduit", description: "Description traduite (facultatif)", remove: "Supprimer" },
  vi: { add: "＋ Thêm tiêu đề bằng ngôn ngữ khác", title: "Tiêu đề đã dịch", description: "Mô tả đã dịch (không bắt buộc)", remove: "Xóa" },
  de: { add: "＋ Titel in einer anderen Sprache hinzufügen", title: "Übersetzter Titel", description: "Übersetzte Beschreibung (optional)", remove: "Entfernen" },
  ru: { add: "＋ Добавить название на другом языке", title: "Переведённое название", description: "Переведённое описание (необязательно)", remove: "Удалить" },
  id: { add: "＋ Tambahkan judul dalam bahasa lain", title: "Judul terjemahan", description: "Deskripsi terjemahan (opsional)", remove: "Hapus" },
  pt: { add: "＋ Adicionar título em outro idioma", title: "Título traduzido", description: "Descrição traduzida (opcional)", remove: "Remover" },
  hi: { add: "＋ दूसरी भाषा में शीर्षक जोड़ें", title: "अनुवादित शीर्षक", description: "अनुवादित विवरण (वैकल्पिक)", remove: "हटाएँ" },
  tr: { add: "＋ Başka dilde başlık ekle", title: "Çevrilmiş başlık", description: "Çevrilmiş açıklama (isteğe bağlı)", remove: "Sil" },
  th: { add: "＋ เพิ่มชื่อเรื่องภาษาอื่น", title: "ชื่อเรื่องที่แปลแล้ว", description: "คำอธิบายที่แปลแล้ว (ไม่บังคับ)", remove: "ลบ" },
  ar: { add: "＋ إضافة عنوان بلغة أخرى", title: "العنوان المترجم", description: "الوصف المترجم (اختياري)", remove: "حذف" },
  bn: { add: "＋ অন্য ভাষায় শিরোনাম যোগ করুন", title: "অনূদিত শিরোনাম", description: "অনূদিত বিবরণ (ঐচ্ছিক)", remove: "মুছুন" },
};
...
function WorldcupMaker({
  ...
  const changeContentLanguageAndPage = (nextLanguage) => {
    ...
    setTitleTranslationLanguages((current) =>
      current.filter((code) => code !== nextLanguage)
    );
  ...
  const otherLanguageCopy =
    OTHER_LANGUAGE_TITLE_COPY[contentLanguage] ||
    OTHER_LANGUAGE_TITLE_COPY.en;

  const getAvailableTranslationLanguage = (rows = titleTranslationLanguages) => {
    const used = new Set([contentLanguage, ...rows]);
    return CONTENT_LANGUAGE_OPTIONS.find((item) => !used.has(item.value))?.value || "";
  };

  const addTitleTranslation = () => {
    const nextLanguage = getAvailableTranslationLanguage();
    if (!nextLanguage) return;

    setTitleTranslationLanguages((current) => [...current, nextLanguage]);
    setTitleTranslations((current) => ({
      ...current,
      [nextLanguage]: current[nextLanguage] || "",
    }));
  ...
  const changeTitleTranslationLanguage = (previousLanguage, nextLanguage) => {
    if (
      !nextLanguage ||
      previousLanguage === nextLanguage ||
      nextLanguage === contentLanguage ||
      titleTranslationLanguages.includes(nextLanguage)
    ) {
      return;
    }

    setTitleTranslationLanguages((current) =>
      current.map((code) => (code === previousLanguage ? nextLanguage : code))
    );

    setTitleTranslations((current) => {
      const next = { ...current };
      const value = next[previousLanguage] || "";
      delete next[previousLanguage];
      next[nextLanguage] = value;
      return next;
    });
  ...
  const removeTitleTranslation = (languageCode) => {
    setTitleTranslationLanguages((current) =>
    ...
    {t("local_video_not_supported") ||
      ...
      "Local video uploads are not supported. You can use YouTube links instead."}
        ...
        >
          ...
          >
            {otherLanguageCopy.add}
          </button>

          {titleTranslationLanguages.length > 0 && (
            <div
              style={{
                marginTop: 10,
                display: "flex",
            ...
            >
              {titleTranslationLanguages.map((languageCode) => {
                const otherUsed = new Set(
                  titleTranslationLanguages.filter((code) => code !== languageCode)
                );
                  ...
                  >
                    ...
                    >
                      <select
                        value={languageCode}
                        onChange={(event) =>
                          changeTitleTranslationLanguage(
                            languageCode,
                            event.target.value
                          )
                        }
                      ...
                      <input
                        type="text"
                        value={titleTranslations[languageCode] || ""}
                        onChange={(event) =>
                          setTitleTranslations((current) => ({
                            ...current,
                      ...
                      <button
                        type="button"
                        onClick={() => removeTitleTranslation(languageCode)}
                        disabled={loading}
                        style={{
                          minHeight: 42,
                          padding: "0 12px",
                      ...
                      >
                        {otherLanguageCopy.remove}
                      </button>
                    </div>

                    <textarea
                      value={descriptionTranslations[languageCode] || ""}
                      onChange={(event) =>
                        setDescriptionTranslations((current) => ({
                          ...current,
                          [languageCode]: event.target.value,
                      ...
                      placeholder={otherLanguageCopy.description}
                      maxLength={400}
                      rows={2}
                      disabled={loading}
                      style={{
