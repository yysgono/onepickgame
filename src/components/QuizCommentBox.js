...
export default function QuizCommentBox({ quizId }) {
  ...
  async function remove(id){ if(!window.confirm(t("comment.deleteConfirm", { defaultValue: "Delete this comment?" }))) return; await supabase.from("quiz_comments").delete().eq("id",id).eq("user_id",user?.id||""); await load(); }
  ...
  return <div style={{maxWidth:900,margin:"34px auto 50px",border:"1.5px solid #cfd8e6",borderRadius:12,padding:16,background:"#fff"}}>
    ...
    <div style={{marginTop:16}}>{loading?<div style={{padding:18,color:"#68758a"}}>{t("comment.loading", { defaultValue: "Loading..." })}</div>:comments.map(c=><div key={c.id} style={{padding:"12px 4px",borderTop:"1px solid #e1e6ee"}}><div style={{display:"flex",justifyContent:"space-between",gap:10}}><strong>{c.nickname||"User"}</strong><span style={{fontSize:12,color:"#7b8798"}}>{new Date(c.created_at).toLocaleString()}</span></div><div style={{whiteSpace:"pre-wrap",marginTop:7,lineHeight:1.5}}>{c.content}</div>{user?.id===c.user_id&&<button type="button" onClick={()=>remove(c.id)} style={{marginTop:7,border:0,background:"transparent",color:"#b42346",padding:0,cursor:"pointer"}}>{t("delete")||"삭제"}</button>}</div>)}</div>
  </div>;
}
